...
import 'package:v_one_mshwar_app_customer/features/payment/wallet/model/paypalClientToken.dart';
...
class PayPalClientTokenGen {
  static Future<PayPalClientTokenModel> paypalClientToken(
    ...
    final response = await http.post(
      ...
      body: {
        "environment": payPal!.isLive.toString() == "true"
            ? "production"
            : "sandbox",
    ...
    showLog("API :: URL :: $url");
    showLog(
      "API :: Request Body :: ${jsonEncode({"environment": payPal.isLive.toString() == "true" ? "production" : "sandbox", "merchant_id": payPal.merchantId.toString(), "public_key": payPal.publicKey.toString(), "private_key": payPal.privateKey.toString()})} ",
    );
    showLog(
      "API :: Request Header :: ${jsonEncode({'apikey': API.apiKey, 'accesstoken': Preferences.getString(Preferences.accesstoken)})} ",
    );
  ...
  static Future paypalSettleAmount({
    ...
    showLog("API :: URL :: $url");
    showLog(
      "API :: Request Body :: ${jsonEncode({"environment": payPal!.isLive.toString() == "true" ? "production" : "sandbox", "merchant_id": payPal.merchantId.toString(), "public_key": payPal.publicKey.toString(), "private_key": payPal.privateKey.toString(), "nonceFromTheClient": nonceFromTheClient, "amount": amount, "deviceDataFromTheClient": deviceDataFromTheClient})} ",
    );
    showLog(
      "API :: Request Header :: ${jsonEncode({'apikey': API.apiKey, 'accesstoken': Preferences.getString(Preferences.accesstoken)})} ",
    );
