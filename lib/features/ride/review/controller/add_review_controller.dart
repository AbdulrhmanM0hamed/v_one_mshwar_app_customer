...
import 'package:v_one_mshwar_app_customer/features/ride/review/model/review_model.dart';
  ...
  Future<dynamic> getReview() async {
    try {
      ...
      final response = await http.get(
        Uri.parse(
          "${API.getRideReview}?user_id=${Preferences.getInt(Preferences.userId)}&ride_id=${rideData.value.id}&review_of=driver&ride_type=${rideType.value}",
        ),
        headers: API.header,
      );
      showLog(
        "API :: URL :: ${API.getRideReview}?user_id=${Preferences.getInt(Preferences.userId)}&ride_id=${rideData.value.id}&review_of=driver&ride_type=${rideType.value}} ",
      );
